...
quantification of pairs of variants in RNA-Seq data.\\
It can be used to study splice variants, where the two variants of the pair 
differ by the inclusion/exclusion of an exonic region.
...
The statistical framework is similar to \Biocpkg{DESeq2}. Counts are modelled 
using the negative binomial distribution. We use the framework of the 
generalised linear model, and we test for association of a variant with a 
condition using a likelihood ratio test.\\
...
function can be directly applied on the output files from \kp~or \krg. Still, 
\kde~can also run with any other software which produces count data as long as 
this data is properly formatted.\\
The user must keep in mind that \kde~was designed to work with at least two 
...
a condition and its 2 replicates. There can be more replicates and more 
conditions. It is not mandatory to have an equal number of replicates in each 
condition.\\
...
These commands require an external output file of \kp~called 
\file{output_kissplice.fa} (which is not included in this package). 
To deal with other types of input file, please refer to section 
\ref{subsec:input}.
The used funtions \Rfunction{kissplice2counts}, \Rfunction{qualityControl}, 
\Rfunction{diffExpressedVariants} and \Rfunction{writeOutputKissDE} of
\kde~are detailed in section \ref{sec:workflow}. For each function, default 
values are used.
...
Be aware that the functions \Rfunction{kissplice2counts} and 
...
The first item to prepare is the condition vector that describes the order of 
the columns in the count table. Considering an example where there are 2 
conditions and 2 replicates per condition, the condition vector will be:

<<conditionVector_howto, fig=FALSE, keep.source=FALSE>>=
myConditions <- c(rep("condition_1", 2), rep("condition_2", 2))
@

because the counts have been input in an order that represents first the 
two replicates of $condition\_1$, then the two replicates of $condition\_2$.\\
...
\item The second column (\Rcode{eventsLength}) contains the variant effective 
size in nucleotides (bp). This information is used only in the context of 
alternative splicing. In the context of SNVs, it can be set to 0. It is used 
to assess which splice variants may induce a frameshift (the difference of 
length between the inclusion and exclusion variant is not a multiple of 3). 
It is also used to precisely estimate the PSI (Percent Spliced In). 
For this purpose, we need the effective size, which corresponds to the number 
...
junction, it corresponds to readLength - 2 * overhang +1 (with the overhang 
corresponding to the minimal number of bases needed to accept that a read is 
aligned to a junction). For the inclusion variant (1st line), it corresponds 
to effectiveLengthExclusion + variablePartLength (with variablePartLength 
being the length of the region only present in the inclusion variant).
...
inclusion variant is 2*effectiveLengthExclusion.
...
each sample. In the example above, \Rcode{cond1rep1} is the number of reads 
supporting this variant in the first replicate of $condition\_1$, 
\Rcode{cond1rep2} is the number of reads sequenced for replicate 2 in 
...
Events are reported in blocks of 4 lines, the two first are about one variant 
of the splicing event (or one allele of the SNV), the following two lines 
about the other variant (or the other allele). 
For each variant, there is a header beginning with the \Rcode{>} symbol 
and a sequence. 
...
\Rcode{Type\_0} would correspond to SNVs.
\end{itemize}
...
\item \Rcode{AS1\_1|SB1\_1|S1\_0|ASSB1\_0|...} and 
\Rcode{AB1\_21|AB2\_12|AB3\_12|AB4\_2|AB5\_5|...} summarize the counts found 
by \kp~quantification step. Here \kp~was run with the option \Rcode{counts} 
set to 2. For the upper path, we have 4 counts for each sample (the different 
reads categories are shown on Figure \ref{fig:readstype}), and for the lower 
path, we have 1 count per sample. There are 8 sets of counts because we gave
8 files in input to \kp. Each count corresponds to the reads coming from each 
file that could be mapped on the variant, in the order they have been
passed to \kp.
\end{itemize}
...
questions about \kp~format and output.\\

To be used in \kde, \kp's output must be converted into a table of counts. 
This can be done with the \Rfunction{kissplice2counts} function.
In the example below, a \kp's output recorded in a file called
\file{output_kissplice_alt_splicing.fa} is loaded.
...
and one count for each variant. If this is not the case, some parameters need 
to be adapted.\\
First, the \Rcode{counts} parameter of \Rfunction{kissplice2counts} must be 
...
Then, the user should also specify the \Rcode{pairedEnd} parameter in 
\Rfunction{kissplice2counts}. 
If RNA-Seq libraries are single-end, \Rcode{pairedEnd} should be set to 
\Rcode{FALSE} (the default value). 
...
cond1\_sample1\_read1, cond1\_sample2\_read1, cond2\_sample1\_read1, 
cond2\_sample2\_read1, cond1\_sample1\_read2, cond1\_sample2\_read2, 
cond2\_sample1\_read2 and cond2\_sample2\_read2.\\
...
data using the \Rcode{k2rg} parameter. 
...
The \Rcode{k2rg} parameter is a boolean value, set to \Rcode{TRUE} if the data
comes from \krg~output.
...
in order to speed up \kde's running time and improve statistical power for 
the selected events. To do this, the \Rfunction{kissplice2counts} function 
...
The events names should come from this list: \Rcode{deletion}, 
\Rcode{insertion}, \Rcode{IR}, \Rcode{ES}, \Rcode{altA}, \Rcode{altD}, 
\Rcode{altAD}, \Rcode{alt}, \Rcode{unclassified}. 
...
\Rcode{remove} vector should countain names from this list: \Rcode{MULTI}, 
\Rcode{altA}, \Rcode{altD}, \Rcode{altAD}, \Rcode{alt}.
By default, all types of events are analysed.

If we want to analyse only cassette exon events (i.e., a single exon is 
...
\begin{itemize}
\item{a heatmap of the sample-to-sample distances (see left panel of
Figure \ref{fig:qualityControl})}
\item{the factor map formed by the first two axes of a principal 
component analysis (PCA) (see right panel of Figure \ref{fig:qualityControl})}
\end{itemize}
...
Indeed, in the heatmap plot, the samples that cluster together are from 
the same condition. 
...
The \Rfunction{diffExpressedVariants} function has three other parameters 
to change the filters or the flags applied on data, and one other parameter 
...
vectors containing p-values before and after correction for multiple testing.
...
see respectively section \ref{subsubsec:finaltable} and section 
\ref{subsubsec:psitable}.\\

To control that the conditions of application of the Benjamini Hochberg 
multiple testing correction procedure are fulfilled, the histogram of the 
p-values before correction can be plotted by using the following command:
...
Because the dataset used here is small, the distribution obtained is not 
representative. Representative histograms can be obtained with the two 
complete datasets presented in the case studies (section 
\ref{sec:casestudies}). As expected, the histograms show a uniform 
distribution with a peak near 0 (Figure \ref{fig:distribpvalue}).
...
as described in section \ref{subsec:flag}. A \Rcode{FALSE} value means 
that the event has high counts (counts above the chosen threshold).
\end{itemize}
...
\paragraph{}To save results, a tab-delimited file can be created using the 
...
to save only events passing thresholds, in a file called 
\file{kissDE_K2RG_results_table.tab}:
...
The goal of this normalization step is to remove technical factors that 
could influence the differential analysis.
By using \Biocpkg{DESeq2} normalization, we correct for library size, 
...
The \Biocpkg{DSS} package (and, to our knowledge, every other packages
estimating the overdispersion of the Negative Binomial model) is suited 
for differential expression analysis (one counts per sample). 
...
\end{tabular}
\caption{Example of en event filtered out before the differential analysis, 
...
The LRT is performed with one degree of freedom, which  corresponds to the
supplementary interaction parameter. 
p-values are then adjusted with a 5\% false discovery rate (FDR) following 
a Benjamini-Hochberg procedure to account for multiple testing, 
...
Indeed, $variant_1$ always have a length greater than or equal to the
$variant_2$, that's why we divide the normalized number of reads of the
...
individual PSI/f is not computed.
\item Then mean PSI/f are computed for each condition. If more than half of
...
of a condition could not be estimated, $\Delta$PSI/$\Delta$f is calculated 
subtracting one condition PSI/f from another.
This does not always follow the order of the conditions the user entered in 
the first place: the conditions are ordered alphabetically. 
To check the final order of the conditions, the output file must be consulted.
$\Delta$PSI/$\Delta$f are calculated subtracting the first condition appearing 
in the header from the one after. $\Delta$PSI/$\Delta$f absolute value vary 
between 0 and 1, with values close to 0 indicating low effects and values
close to 1 strong effects.
\end{itemize}
...
\label{subsec:SNV}
RNA-Seq data can be used to study SNVs (SNPs, mutations, RNA editing) in 
expressed regions of the genome. 
Such SNVs can be detected and reported by \kp. 
In this first following example, an analysis of SNPs is done with \kde~on 
RNA-Seq data from the human GEUVADIS project \cite{Lappalainen2013}. 
The sample data presented here is a subset of the case study presented in
...
In total, 8 files have been used as \kp~input: 4 files for the two TSC 
...
corresponds to the SNV, here A/C at position 42.
...
\file{output_kissplice_SNV.fa}) into a format that will be used in \kde's
main functions, thanks to the \Rfunction{kissplice2counts} function.\\
\bioccomment{\Robject{fileIn} contains the absolute path of the file on the 
user's hard disk.}\\
Due to paired-end RNA-Seq data, the \Rcode{pairedEnd} parameter was set to 
\Rcode{TRUE}.

This conversion in a table of counts is stored in a \Robject{myCounts\_SNV} 
...
plan must also be provided. 
In the example, there are two replicates of TSC and two replicates of CEU.
So the \Robject{myConditions\_SNV} vector is defined:
...
Before running the differential analysis, we check if the data was correctly
loaded, by running the \Rfunction{qualityControl} function.
...
\ref{fig:qcSNV}), the replicates of the same condition seem to be more similar
between themselves than to the samples of the other condition. 
On the heatmap (left of Figure \ref{fig:qcSNV}), the samples of the same
...
(\Rcode{Adjusted\_pvalue} column, equal to \Rcode{4.975013e-09}) and a very 
contrasted $\Delta f$ (\Rcode{Deltaf/DeltaPSI} column, equal to 
\Rcode{-0.9258}) close to the maximum value (1 in absolute). 
...
than 5\%. Results passing this threshold are thus extracted from 
\Robject{myResults\_SNV} thanks to the \Rcode{adjPvalMax} parameter and are 
saved in a \file{final_table_significants.tab} file, thanks to the 
...
This second example corresponds to the case of differential analysis of 
...
The data used in this example of AS comes from the ENCODE project 
\cite{Djebali2012}. 
The chosen samples are from a neuroblastoma cell line, SK-N-SH, with 
...
It assigns each event to the gene it maps to and to a type of alternative 
splicing event: Exon Skipping (ES), Intron Retention (IR), Alternative Donor 
(AltD), Alternative Acceptor (AltA), \dots. 
For further questions on these tools (\kp~ and \krg), please refer to the 
...
Following is an extract of this file (the first 3 rows and first 11 columns),
where each row is one alternative splicing event:
...
The \Rfunction{kissplice2counts} function allows to transform this text file
(here called \file{output_k2rg_alt_splicing.txt}) into a format compatible 
with \kde's main functions.\\
\bioccomment{\Robject{fpathinAS} contains the absolute path of the file on 
the user's hard disk.}\\
As these samples are paired-end, the \Rcode{pairedEnd} parameter is set to 
\Rcode{TRUE}.
The \Rcode{k2rg} parameter must be set to \Rcode{TRUE} to indicate that the 
file comes from \krg~and not directly from \kp. 
The \Rcode{counts} parameter must be set to the same value (i.e., 2) used in 
\kp~and \krg~to indicate which counts are given in input.
...
This conversion in a table of counts is stored in a \Robject{myCounts\_AS} 
object (for a detailed description of its structure, see section 
\ref{subsubsec:input_krg}) and can be done as follows:
...
To perform the differential analysis, a vector that describes the experimental 
plan must also be provided. 
In the example, there are two replicates of the SK-N-SH cell line without 
treatment (SKNSH) and two replicates of the same cell line treated with
...
and the result for the first three events is printed:
...
\Rcode{-0.8042}) close to the maximum value (1 in absolute). 
...
The data conversion with the \Rfunction{kissplice2counts} function and the 
statistical analysis done with the \Rfunction{diffExpressedVariants} function 
can take some time to run. Here is an example of the running time of these 
two functions on the two complete datasets presented in the case studies
(section \ref{sec:casestudies}). The time presented were evaluated on a
desktop computer with the following caracteristics: 
Intel Core i7, CPU 2,60 GHz, 16G RAM.
