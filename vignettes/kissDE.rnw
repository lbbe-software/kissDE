...
\bioctitle{kissDE Reference Manual}
...
The statistical framework is similar to \Biocpkg{DESeq}. Counts are modelled using the negative binomial distribution. We use the framework of the generalised linear model, and we test for association of a variant with a condition using a likelihood ratio test.\\
...
<<kissplice_format, echo=TRUE, keep.source=FALSE>>=
