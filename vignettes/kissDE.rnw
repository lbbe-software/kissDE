...
\section{Examples}
\subsection{Installation and library loading}
<<install>>=
...
<<tablecounts_howto>>=
tableCounts <- read.table("table_counts_alt_splicing.txt")
head(tableCounts)
@
\begin{itemize}
...
myCounts <- kissplice2counts("output_kissplice_alt_splicing.fa")
names(myCounts)
...
\kde contain a function that allow the user to control the quality of the data and to check if no error was done at the data loading step.
The function \texttt{qualityControl} produce 3 plots :
\begin{itemize}
\item{dendrogram}
\item{heatmap of the sample-to-sample distances}
\item{comparison of the variance inter-group and intra-group}
\end{itemize}
<<qualityControl_howto>>=
qualityControl(myCounts,myConditions)
 @
With the dendrogram and the heatmap, the user can check if a replicate seem to be very different from the others.
