...
The input count data to \kde~may come from \kp~\cite{KisSplice}, \krg~or any other software which produces count data properly formatted.
As \kde~was first designed to be a brick of the KisSplice pipeline (web page: \url{http://kissplice.prabi.fr/}), the \Rfunction{kissplice2counts} function can be directly applied on the output files from \kp~or \krg.\\
...
These commands require an example output file of \kp~called \file{output_kissplice.fa}. 
To deal with other types of input file, please refer to section \ref{subsec:input}.
The used funtions \Rfunction{kissplice2counts}, \Rfunction{qualityControl}, \Rfunction{diffExpressedVariants} and \Rfunction{writeOutputKissDE} of \kde~are detailed in section \ref{sec:workflow}.
