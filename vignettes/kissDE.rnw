...
It assigns each event a type of alternative splicing (Exon Skipping (ES), 
Intron Retention (IR), Alternative Donor (AltD), Alternative Acceptor 
(AltA), \dots) and the gene the event maps to.
...
Bellow is an extract of this file (the first 3 rows and first 11 columns),
