...
\kde~is a R-package for the analysis of RNA-seq data in the context of alternative splicing. It is the the continuation of \kp~but may also be used independently with data from other software to search. \kde~detects genomic variants like exon specific events, indels, SNP/mutations, that are specific to a biological condition. Many experiments include several biological conditions to decypher their influence on the present transcripts. After one have found a collection of variants by analyzing its reads, it is natural to focus on the biological relevance of what is in the list, which can be thousands of variants. \kde~helps to sort interesting variants by testing if a variant is specific to a condition.
