...
\subsection{Provide a table of counts, use of kissplice2counts}
If you use output files from KisSplice, a fist step of conversion is required using \texttt{kissplice2counts} function.
To create an object with \texttt{kissplice2counts}, you will often use one of these lines:\\
...
\subsection{Provide an experimental conditions vector}
...
What is important is that this vector matches the order of what you have input. Probably it will be clearer if you give \texttt{condition1} and \texttt{condition2} a more descriptive name. 
...
\section{Tests}
We use a negative binomial distribution to represent the read counts as in standard RNA-Seq analysis. We test for significant interaction between \textit{variants} and \textit{experimental conditions} as main effects, with a generalised linear model framework.
\section{Pvalues}
Pvalues are adjusted for multiple testing (FRD=5\%).

\section{Delta PSI (delta f if SNP) computation}
The  percent spliced in (PSI) is not used to perform tests but we output it to give information about the size of the effect. Indeed, with highly expressed genes pvalues can be significant even if for smal switches of variant presence, because there are a lot of reads to assess it. Intuitively percent spliced in are the ratio of the quantity of counts witnessing the presence of the inclusion isoform over the total counts sum for both isoforms. The delta PSI is simply the subtraction of the PSI calculated for each condition. A high delta PSI (in absolute) shows that the main isoform has changed between the two conditions.\\ 
However this value is biased because the inclusion isoform is longer (in case of indels and alternative splicing) and more reads are prone to maping on it. That is why we introduced a correction of the counts to compute the delta PSI:
\begin{itemize}
