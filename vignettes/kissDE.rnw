...
\author{Lilia Brinza, Vincent Lacroix, Frank Picard, Vincent Miele and Camille Marchet}
...
The user must keep in mind that \kde was designed to work with at least one replicate for each condition, which means that the minimal input is variants quantified for 4 different individuals, two that represent .\\
