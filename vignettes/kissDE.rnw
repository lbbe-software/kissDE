...
In the case where the input data contains more than 2 conditions, we advise the user
to remove samples from the analysis in order to compare 2 conditions only, because 
\kde was uniquely tested in this context. To remove samples from the analysis the "*"
