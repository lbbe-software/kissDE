...
head(dataframekissDE)
                  X0    X1   X2  X3  X4  X5
1     variant1_gene1   127  955 897 302 443
2     variant2_gene1    80   17  20   5   2
3     variant1_gene2  1543  450 599 400 325
4     variant2_gene2  1201    1   5   4   0
5     variant1_gene3   221 1192 881 479 587
6     variant2_gene3   219   83  48  50  37
...
\end{verbatim}
\begin{itemize}
 ...
 For alternative splicing events, the inclusion form and the exclusion forms has one line each.
 \item \textbf{Columns:}
 \begin{itemize}
 \item Each line has a first column (\texttt{X0}) with the name of the variation.
 \item The second column (\texttt{X1}) is the size of the variant. Is this size is unknown, the value can be set to 0. Note that the size is used only in the case of alternative splicing, to give the indication wether the size difference due to the switch from one isoform to the other is a multiple of 3 or not (thus induces a switch in the ORF).
 \item  All others columns (\texttt{X2... Xn}) show counts for each replicate in each condition for the variant. For instance, let's assume we work with two conditions $(condition_1, condition_2)$. \verb|X2| is the counts corresponding to reads from the first replicate in $condition_1$, \verb|X3| is the counts of reads sequenced for replicate 2 in $condition_1$, \verb|X4| and \verb|X5| are counts for replicates 1 and 2 of $condition_2$.
 \end{itemize}
...
 \verb|myConditions <- c(rep("condition_1",2),rep("condition_2",2))|\\
 because (\texttt{X2, X3, X4, X5}) have been put in an order that represents first two replicates of $condition_1$, then two replicates of $condition_2$.
\end{itemize}
