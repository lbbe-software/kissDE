...
plotPSI <- function(diffVariants, conditions, thresholdPvalue = 0.05, thresholdDeltaPSI = 0.10, show = "PSI", storeFigs = FALSE){
  ...
  if (storeFigs == FALSE) {
    ...
    if (show == "PSI" || show == "both"){
      plot(x=c(1:length(new_data_ordered[,columnName])), y=new_data_ordered[,columnName], ylab="Percent Spliced In (PSI)", col=colors[[1]][new_data_ordered$signif], pch=point_type[new_data_ordered$signif], ylim=c(0,1), xaxt='n', xlab="")
      for (i in 1:n){
        for (j in 1:nr[i]){
          if (i != 1 || j != 1){
            columnName <- paste(uniqconditions[i],"_repl",j,sep="")
            points(x=c(1:length(new_data_ordered[,columnName])),y=new_data_ordered[,columnName], col=colors[[i]][new_data_ordered$signif], pch=point_type[new_data_ordered$signif])
          }
        }
        ...
        if (show == "both"){
          ...
          points(x=c(1:length(new_data_ordered[,columnNameMean])), y=new_data_ordered[,columnNameMean], col=colors[[i]][new_data_ordered$signif], pch=point_type_mean[new_data_ordered$signif])
      ...
      for (i in 1:n){
        columnNameMean <- paste("Mean_PSI_",uniqconditions[i],sep="")
        if (i == 1){
          plot(x=c(1:length(new_data_ordered[,columnNameMean])), y=new_data_ordered[,columnNameMean], ylab="Percent Spliced In (PSI)", col=colors[[i]][new_data_ordered$signif], pch=point_type_mean[new_data_ordered$signif], ylim=c(0,1), xaxt='n', xlab="")
        }
        else{
          points(x=c(1:length(new_data_ordered[,columnNameMean])), y=new_data_ordered[,columnNameMean], col=colors[[i]][new_data_ordered$signif], pch=point_type_mean[new_data_ordered$signif])
        }
