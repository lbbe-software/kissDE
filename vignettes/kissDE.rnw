...
The created graphs can be saved by setting the \Rcode{storeFigs} parameter of the \Rfunction{qualityControl} function to \Rcode{TRUE} (then graphs are stored in a \file{kissDEFigures} folder created in the \R{} working directory) or to the path where the user wants to store his/her graphs.
