...
are shown on Figure \ref{fig:readstype}. There are 8 sets of counts because we 
gave 8 files in input to \kp. Each count corresponds to the reads coming from 
each file that could be mapped on the variant, in the order they have been
passed to \kp.
\end{itemize}
