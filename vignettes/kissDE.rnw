...
gave 8 files in input to \kp (denotated by the number before the "_" character). 
Each count (denotated by the number after the "_" character) corresponds to the 
reads coming from each file that could be mapped on the variant, in the order 
they have been passed to \kp.
\end{itemize}
