...
The workflow for SNPs/SNVs is fully described in Lopez-Maestre et al., the workflow for splicing is fully described in Benoit-Pilven et al.
\end{abstract}
...
The used funtions \texttt{kissplice2counts}, \texttt{qualityControl}, \texttt{diffExpressedVariants} and \texttt{writeOutputKissDE} of \kde~are detailed in the section \ref{sec:workflow}.
...
To be used in \kde, \kp's output must be converted in a table of counts thanks to the \texttt{kissplice2counts} function.
...
\item The second column (\texttt{events.length}) contains the size of the variant, extracted from the \kp~header. It indicates whether the size difference due to the switch from one isoform to the other is a multiple of 3 or not (thus induces a frameshift in the ORF).
...
\krg~allows the annotation of the alternative splicing events. It assigns each event to the gene it maps to and to a type of alternative splicing event: Skipped Exon (SE), Intron Retention (IR), Alternative Donor (AltD), Alternative Acceptor (AltA). Interested users should refer to \krg~manual for further questions about \krg~format and output (\url{ftp://pbil.univ-lyon1.fr/pub/logiciel/kissplice/tools/kissplice2refgenome_userguide.pdf}).
...
\begin{itemize}
\item{a heatmap of the sample-to-sample distances (see left of Figure \ref{fig:qualityControl})}
...
Because the dataset used here is small, the distribution obtained is not representative. Representative histogram can be obtained with the two complete datasets presented in the case studies (section \ref{sec:casestudies}). As expected, the histograms show an uniform distribution with a peak near 0 Figure \ref{fig:distribpvalue}).
...
The \texttt{f/psiTable} output of the function \texttt{diffExpressedVariants} contains the f value for SNV analysis and PSI value for alternative splicing analysis (see details and computation in section \ref{subsec:psi}) for each event in each sample.
