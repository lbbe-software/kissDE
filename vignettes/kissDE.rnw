...
If you use the output files from \kp~or \krg, you can directly import your counts using \Rfunction{kissplice2counts} function.\\
...
\warning{To run \kde, all conditions must have replicates. So each condition must at least be present twice in the condition vector. If this is not the case, you will get an error message.}
...
An input example table contained in a flat file called \file{table_counts_alt_splicing.txt} (\Robject{fpath1} contains the absolute path of the file on your hard disk) is loaded and stored in a \Robject{tableCounts} object.
...
In the example below, a \kp's output recorded in a file called \file{output_kissplice_alt_splicing.fa} (\Robject{fpath2} contains the absolute path of the file on your hard disk) is loaded.
...
In the example below, a \krg's output recorded in a file called \file{output_k2rg_alt_splicing.txt} (\Robject{fpath3} contains the absolute path of the file on your hard disk) is loaded.
...
\begin{itemize}
  \item \Rcode{pvalue}: By default, the p-value threshold to output the significant events is set to 1. So all variants are output in the final table. This parameter must be a numeric value between 0 and 1. Be aware that by setting \Rcode{pvalue} to 0.05, only events that have been identified as significant between the conditions with a false discovery rate (FDR) $\leqslant$ 5\% will be present in the final table. If you wish to change this threshold, you will need to recompute everything.
...
The first step is to convert this fasta file (here called \file{output_kissplice_SNV.fa}, available at the \Robject{fileIn} path in your hard disk) into a format that will be used in \kde's main functions, thanks to the \Rfunction{kissplice2counts} function. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Load data}
The \Rfunction{kissplice2counts} function allows to transform this text file (here called \file{output_k2rg_alt_splicing.txt}, available at the \Robject{fileInAS} path in your hard disk) into a format compatible with \kde's main functions. 
