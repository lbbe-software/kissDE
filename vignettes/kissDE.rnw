...
\begin{itemize}
  ...
  \item \Rcode{Type\_1} means that the sequences correspondto a splicing event. \Rcode{Type\_0} would correspond to SNVs.
\end{itemize}
...
The \Rfunction{diffExpressedVariants} function has three other parameters to change the filters or the flags applied on data, and one other parameter to indicate if the data are from technical replicates:
\begin{itemize}
  \item \Rcode{pvalue}: By default, the p-value threshold to output the significant events is set to 1. So all variants are output in the final table. This parameter must be a numeric value between 0 and 1. Be aware that by setting \Rcode{pvalue} to 0.05, only events that have been identified as significant between the conditions with a false discovery rate (FDR) > 5\% will be present in the final table. If you wish to change this threshold, you will need to recompute everything.
...
\Rcode{resultFitNBglmModel} is a data frame containing the results of the fitting of models to the data.
...
For example, if we want to save in a file called \file{results_table_filtered.tab} only events with the adjusted p-value < 0.05 and the Deltaf/DeltaPSI absolute value > 0.10, the following command can be used:
...
The \Rcode{f/psiTable} output of the \Rfunction{diffExpressedVariants} function contains the f values for SNV analysis and PSI values for alternative splicing analysis (see details and computation in section \ref{subsec:psi}) for each event in each sample.
...
To use this information with external tools, this table can be saved in a tab-delimited file (here called \file{result_PSI.tab}), setting the \Rcode{writePSI} parameter to \Rcode{TRUE}:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Model}
Then we design two models to take into account interactions with variants (SNVs or alternative isoforms) and experimental conditions as main effects. We  use the generalised linear model framework. The expected intensity $\lambda_{ijk}$ can be written as in following:
\begin{equation}
...
Outputs are stored in a \Robject{myResult\_SNV} object (for a detailed description of its structure, see section \ref{subsubsec:finaltable}) and the sequence of successive events is printed:
...
So the \Robject{myConditions\_AS} vector is defined:
...
\subsubsection{Differential analysis}
The main function of the \kde, \Rfunction{diffExpressedVariants}, can now be run to compute the statistical test. 
Outputs are stored in a \Robject{myResult\_AS} object (for a detailed description of its structure, see section \ref{subsubsec:finaltable}) and the sequence of successive events is printed:
