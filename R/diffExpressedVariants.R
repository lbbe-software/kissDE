...
diffExpressedVariants <- function(countsData, conditions, storeFigs=FALSE, pathFigs="None", pvalue=0.05, filterBoundary=10, flagLowCountsBoundary=10) {
  ...
  exprs(dispData) <- exprs(dispData)[-which(totUP<filterBoundary & totLOW<filterBoundary),]
  exprs(dispDataMeanCond) <- exprs(dispDataMeanCond)[-which(totUP<filterBoundary & totLOW<filterBoundary),]
