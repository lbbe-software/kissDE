...
The statistical framework is based on similar hypotheses as \Biocpkg{DESeq2} 
...
In a \R{} session, the \CRANpkg{BiocManager} package has first to be installed.
...
\Rcode{IR}, \Rcode{ES}, \Rcode{altA}, \Rcode{altD}, \Rcode{altAD}, \Rcode{indel},
\Rcode{-}.\\
...
both alternative acceptor and donor site (\Rcode{altAD}) or any of the alt
combinaison with MULTI. Thus, in this specific case, the 
\Rcode{remove} vector should contain names from this list: \Rcode{MULTI},
\Rcode{altA}, \Rcode{altD}, \Rcode{altAD}, \Rcode{MULTI\_altA}, 
\Rcode{MULTI\_altD}, \Rcode{MULTI\_altAD}.
...
are stored in a \file{kissDEFigures} folder, created in a temporary directory, 
...
\Rfunction{qualityControl} function is used in an automatized workflow.
...
\item \Rcode{nbCore}: An integer value indicating how many cores should be used
for the computation. This parameter should be strictly lower than the number of 
core of the computer (\Rcode{nbCore} $<$ nbr computer cores $- 1$). By default, 
this parameter is set to 1, meaning that the computation are not parallelized.
\end{itemize}
...
(model $\mathcal{M}(\phi=0)$) is chosen in \kde.\\
...
\subsection{Application of \kde~to alternative splicing}
\label{subsec:AS}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Explore results}

The \Rfunction{writeOutputKissDE} function also write an \Rcode{rds} file in the
output directory, with a \Rcode{.rds} extension. This file can be inputed in the
\Rfunction{exploreResults} function in order to explore and plot the results of
\kde through a Shiny interface:

<<as_explore_result, fig=FALSE, eval=FALSE, keep.source=FALSE>>=
exploreResults(rdsFile = "results_table.tab.rds")
@

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{One command to rule them all}

All of \kde R command line can be run at once with the \Rfunction{kissDE}
function.

<<as_kissDE_result, fig=FALSE, eval=FALSE, keep.source=FALSE>>=
fileInAS <- system.file("extdata", "output_k2rg_alt_splicing.txt",
    package = "kissDE")
myConditions_AS <- c(rep("SKNSH",2), rep("SKNSH-RA",2))
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Application of \kde~to SNPs/SNVs}
\label{subsec:SNV}
This second example present an analysis of SNPs/SNVs done with \kde~on 
...
with pooled data (i.e. individuals are pooled prior to sequencing). Pooling 
can be used to decrease the costs. It is also sometimes the only option, when
too few RNA is available per individual.
The method can in principle be used on unpooled data, polyploid genomes, and 
for the detection of somatic mutations, but has for now only been evaluated 
for the detection of SNPs/SNVs in pooled RNAseq data.\\
In the remaining, we use the term SNV, which designates a variation of a
single nucleotide, without any restriction on the frequency of the two
alleles. The term SNP is indeed classically used for variants present in
at least 1\% of a population.
...
\kp~outputs a fasta file that stores SNVs found in the dataset. Its structure 
...
<<snv_explore_result, fig=FALSE, eval=FALSE, keep.source=FALSE>>=
...
<<snv_kissDE_result, fig=FALSE, eval=FALSE, keep.source=FALSE>>=
fileInSNV <- system.file("extdata", "output_kissplice_SNV.fa", 
    package = "kissDE")
myConditions_SNV <- c(rep("TSC",2), rep("CEU",2))
...
The statistical analysis function (\Rfunction{diffExpressedVariants})  
is the most time-consuming steps. Here is an example of the running time 
of this function on the two complete datasets presented in the case 
...
Running time of  \\
& & events & \footnotesize{\Rfunction{diffExpressedVariants}} \\ \hline
\multirow{3}{*}{AS data} & \Rcode{counts=2},  & \multirow{3}{*}{59132} & 
\multirow{3}{*}{17m} \\ 
...
& \Rcode{pairedEnd=TRUE} & & \\
\end{tabular}
\caption{Profiling. Running time of the principal function of 
\kde~(\Rfunction{diffExpressedVariants})
for two datasets (AS dataset from the ENCODE project \cite{Djebali2012} 
described in section \ref{subsec:AS} and SNV dataset from the GEUVADIS 
project \cite{Lappalainen2013} described in section \ref{subsec:SNV}).}
\label{tab:time}
\end{table}

To reduce even more the running time of \Rfunction{diffExpressedVariants}, the
parameter \Rcode{nbCore} can be used to parallelize the most time-consuming
step of this function (for more detailed explanation on this parameter see
section \ref{subsec:diffanalysis}).
