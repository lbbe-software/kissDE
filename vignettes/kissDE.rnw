...
interessed in intron rentention events, the \Rcode{keep = c("IR")} option
...
multiple testing correction procedure \cite{benjamini1995} are fulfilled, the 
histogram of the p-values before correction can be plotted by using the 
following command:
