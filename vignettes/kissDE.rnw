...
\Rcode{altAD}, \Rcode{alt}, \Rcode{unclassified}. Thus, if the user is only
interessed in intron rentention events, the \Rcod{keep = c("IR")} option
should be used. If the user isn't interessed in deletions and insertions,
the \Rcode{remove = c("insertion", "deletion")} option should be used.
...
If the user want to analyse only cassette exon events (i.e., a single exon is 
