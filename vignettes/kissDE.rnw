...
The sample data presented here is a subset of the case study presented in Lopez-Maestre et al.\cite{Lopez-Maestre2016}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Dataset}
The data used in this example of AS comes from ENCODE\cite{Djebali2012}. 
...
Then, the type 1 results from \kp~were mapped to the reference genome with STAR and analyzed with \krg. 
\krg~enables to annotate the AS events discovered by \kp. 
It assigns each event to the gene it maps to and to a type of alternative splicing event: Exon Skipping (ES), Intron Retention (IR), Alternatid Donor (AltD), Alternative Acceptor (AltA). 
