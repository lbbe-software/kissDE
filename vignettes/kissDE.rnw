...
In the case where input data contains more than 2 conditions, we advise the user
to remove samples from the analysis in order to compare 2 conditions only, because 
\kde was only tested in this context. To remove samples from the analysis the "*"
