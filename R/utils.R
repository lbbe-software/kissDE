...
.readAndPrepareData <- function(countsData, conditions) {
  ...
  if (length(nr[nr == 1]) > 0){
    stop("Your data does not contain replicates for all conditions. 
    		 Please change your input data and relaunch.")
  }
...
.modelFit <-function(countsData, n, nr, ASSBinfo, filterLowCountsVariants){
  ...
  allEventtables <- apply(dataPart2, 1, 
		.eventtable, 
		startPosColumn4Counts=which(grepl("UP", names(dataPart2)))[1], 
  ...
  allEventtables <- apply(dataPart3, 1, 
			.eventtable, 
			startPosColumn4Counts=which(grepl("UP", names(dataPart3)))[1], 
...
.bestModelandSingular <- function(pALLGlobalPhi.glm.nb, sing.events, 
  ...
  if (NROW(pALLGlobalPhi.glm.nb) > 0) {
    ...
    ###################################################
    bestmodel.table.n <- 
    	apply(matrixpALLGlobalPhi[, c(6, 8, 10, 12)], 1, which.min)
    ...
    singhes0 <- 
    	which(apply(pALLGlobalPhi.glm.nb[, c(6, 8, 10, 12)], 1, which.min) == 1)
    ...
    ###################################################
    singhes <- 
    	which(apply(matrixpALLGlobalPhi[, c(6, 8, 10, 12)], 1, which.min) > 1 & 
    					rowSums(matrixpALLGlobalPhi[, c(22, 24, 26, 28)]) != 0)
    ...
    i <- 1  
    li.singhes <- 
    	which(apply(pALLGlobalPhi.glm.nb[, c(6, 8, 10, 12)], 1, which.min) == i)
    ...
    i <- 2
    li <- which(apply(pALLGlobalPhi.glm.nb[, c(6, 8, 10, 12)], 1, which.min) ==
    							i & rowSums(pALLGlobalPhi.glm.nb[, c(22, 24, 26, 28)]) == 0)
...
.sizeOfEffectCalc <- function(signifVariants, ASSBinfo, n, nr, 
  ...
  if (!is.null(ASSBinfo)) {
  	...
  	## signifVariants
    ASSBinfo <- subset(ASSBinfo, 
			ASSBinfo$events.names %in% as.vector(signifVariants[, 1]))
  }
  ...
  lowcounts <- apply(sumLowCond, 1, function(x) length(which(x < 
			flagLowCountsConditions))) >= n - 1
...
.writeMergeOutput <- function(resDiffExpr, k2rgFile, pvalMax=1, 
  ...
  countsLabel <- 
  	paste(colnames(finalTable)[COUNTSSTART:COUNTSEND], collapse=",")
  ...
  while(i <= length(lines)) {
    ...
    if(bcc %in% lBcc) {
      countsValue <- 
      	paste(as.character(
      			finalTable[rownames(finalTable) == bcc, ][COUNTSSTART:COUNTSEND]), 
      		collapse=",")
      ...
      if (dpsi >= dPSImin && apv <= pvalMax) {
        toWrite <- paste(line, countsValue, psiValue, 
					finalTable[rownames(finalTable) == bcc, ]$Adjusted_pvalue,
					finalTable[rownames(finalTable) == bcc, ]$`Deltaf/DeltaPSI`,
					finalTable[rownames(finalTable) == bcc, ]$lowcounts,
					sep="\t")
        writeLines(toWrite,fOut)
      }
    }
