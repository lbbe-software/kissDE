...
In the context of RNA-seq, NGS gives access to important sequencing depth and enables to study different transcripts isoforms coming  from the same gene. In eukaryotes, many genes are not only spliced (
exons are kept to build a mRNA) but also undergo alternative splicing, where combinations of exons in a gene yield different transcripts that correspond to alternative isoforms. Through this process a single gene can produce a collection of RNA molecules. Many experiments include several biological conditions to decypher their influence on the present transcripts. After one have found a collection of variants by analyzing its reads, it is natural to focus on the biological relevance of what is in the list, which can be thousands of variants. 
...
The \texttt{kissplice2count} function allow to transform this texte format into a format compatible with kissDE's main function for testing. The \texttt{r2rg} option must be set to \texttt{TRUE} to indicate that the file comes from KisSplice2refgenome and not from \kp. Like explained earlier, thes samples are paired-end. This information must be given to the \texttt{kissplice2count} function.
% talk about the counts and the exonicReads options

<<AS_counts, tidy=TRUE, highlight=FALSE>>=
...
<<AS_ES_data, tidy=TRUE, highlight=FALSE>>=
...
To be able to perform the differential analysis, we must provide a vector that describes our experimental plan. In this example, we have two replicates of the SK-N-SH cell line without treatment (SKNSH) and two replicates of the same cell line treated with retinoic acid (SKSNH-RA). So we construct a vector:
<<AS_condition>>=
...
With this vector we can call the main function of the package that computes the statistical test. kissDE outputs a list of events that passed the p-value threshold:
<<AS_test>>=
myResultAS <- diffExpressedVariants(myCountsAS, myConditionAS, pvalue = 1)
head(myResultAS$finalTable)
@

% Explain results (differents cases) : significants or not, NA, low counts, ...
