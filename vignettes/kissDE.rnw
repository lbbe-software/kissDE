...
a biological condition and its 2 replicates. There can be more replicates and more 
conditions, but it is not mandatory to have an equal number of replicates in each 
condition.\\
...
\ref{subsec:input}.
The funtions used in \kde~are \Rfunction{kissplice2counts}, \Rfunction{qualityControl}, 
...
For more details on functions and their parameters see section \ref{sec:workflow}.
