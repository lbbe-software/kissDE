...
Below is an example of the first four lines of an example of \kp~output:
...
\item \Rcode{AS1\_1|SB1\_1|S1\_0|ASSB1\_0|AS2\_0|SB2\_0|S2\_0|ASSB2\_0|...} and
\Rcode{AB1\_21|AB2\_12|AB3\_12|AB4\_2|AB5\_5|...} summarizes the counts found 
by \kp~quantification step.
Here \kp~was run with the option \Rcode{counts} set to 2. 
For the upper path, we have 4 counts for each sample: AS, SB, S and ASS. For
the lower path, we have 1 count per sample: AB. The different reads categories
are shown on Figure \ref{fig:readstype}. There are 8 sets of counts because we gave
