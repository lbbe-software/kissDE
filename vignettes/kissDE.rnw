...
Because the dataset used here is small ($\sim$ 100 lines), the histograms of the two 
complete datasets presented in the case studies (section \ref{sec:casestudies})
are represented. As expected, the histograms show a uniform 
...
a Benjamini-Hochberg procedure \cite{benjamini1995} to account for multiple testing, 
